...
import { doc, updateDoc, collection, query, where, getDocs, arrayUnion, arrayRemove } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
import { avatarImages, coverImages } from './pimage.js';
import { processImageUrl } from './gdrive.js'; // Using the new smart processor
...
function renderGrid(containerId, imageArray, currentValue, callback, type, isSavedTab) {
    ...
    if (imageArray.length === 0 && isSavedTab) {
        container.innerHTML = `<p style="color:var(--text-muted); font-size:0.9rem; grid-column:1/-1; padding:20px 0; text-align:center;">No saved links found.</p>`;
        return;
    }
    ...
    imageArray.forEach((imgStyle) => {
        ...
        // Add Delete Button for Saved Links
        if (isSavedTab) {
            const deleteBtn = document.createElement('div');
            deleteBtn.className = 'delete-img-btn';
            deleteBtn.innerHTML = '<i class="ri-close-line"></i>';
            deleteBtn.addEventListener('click', async (e) => {
                e.stopPropagation();
                if(confirm("Erase this link from your library?")) {
                    await deleteCustomImage(type, imgStyle);
                }
            });
            div.appendChild(deleteBtn);
        }
...
function setupTextAvatarCreator() {
    ...
    document.getElementById('apply-text-avatar-btn')?.addEventListener('click', (e) => {
        ...
        btn.innerText = "Applied to Selection Buffers!";
        btn.style.background = "#10b981";
        setTimeout(() => { btn.innerText = "Select this Style"; btn.style.background = "var(--primary-color)"; }, 2000);
    });
}
...
// 🔗 SMART LINK SAVING LOGIC
async function saveCustomLinkToFirebase(type, urlInputId) {
    const input = document.getElementById(urlInputId);
    const btnId = type === 'avatar' ? 'add-avatar-btn' : 'add-cover-btn';
    ...
    // Process the URL (Google Drive / Pinterest / Standard)
    const finalUrl = processImageUrl(rawUrl);
    const formattedUrl = `url('${finalUrl}')`;
    const field = type === 'avatar' ? 'savedAvatars' : 'savedCovers';
    ...
    try {
        ...
        // Auto-select the newly added image so the user can just click "Set as Avatar"
        if (type === 'avatar') tempSelectedAvatar = formattedUrl;
        if (type === 'cover') tempSelectedCover = formattedUrl;

        // Switch automatically to the "Saved" tab
        document.querySelector(`#modal-${type} .tab-btn:nth-child(2)`).click(); 
    } catch (err) { 
        console.error(err);
        alert("Failed to save image link to database."); 
    } finally {
