...
//     updateCreatePostUI();
//     setupMarkdownButtons();
//     // initGlobalPostListeners(); // Activate post clicking logic
//     loadFeed();
// });
...
//             // 2. RESOLVE AUTHOR AVATARS VIA ENGINE
//             renderDynamicAuthor(post.authorId, postId);
//         });
        
//         // Calculate sidebar trends
//         calculateSmartTrends(allPosts, trendingList);
//     });
// }
function loadFeed() {
    // 🔥 FIX: Target the new specific container, not the whole feed!
        const feedContainer = document.querySelector('.main-feed');
    const createPostBox = document.querySelector('.create-post');
    const postsContainer = document.getElementById('feed-posts-container');
    const trendingList = document.querySelector('.trending-list');
    
    // Safety check just in case we are on a page without the feed
    if (!postsContainer) return;

    onSnapshot(query(collection(db, "posts"), orderBy("timestamp", "desc")), (snapshot) => {
        // Now this only clears old posts, leaving the greeting and create box alone!
        ...
        snapshot.forEach((docSnap) => {
            ...
            // 1. GENERATE HTML VIA ENGINE
            postsContainer.insertAdjacentHTML('beforeend', createPostHTML(post, postId, auth.currentUser.uid));
...
// Global routing function for user profile clicks
window.goToProfile = function(userId) {
    if (auth.currentUser && auth.currentUser.uid === userId) window.location.href = 'profile.html';
    else window.location.href = `user.html?id=${userId}`;
}